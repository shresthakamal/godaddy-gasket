...
const label = 'Update next/image components using Next.js codemod';
 ...
 */
async function updateNextImageComponents(
  ...
  if (modifiedFiles.length) {
    modifiedFiles.forEach(({ modified }) => {
      messages.push(
        ...modified.map(
          (f) =>
            `${f} - updated to use the latest version of \`next/image\` component`
        )
      );
    });
    nextSteps.push(`Legacy image components were detected and modified to use the latest next/image.
