  ...
  for (const dirent of packageDirs) {
    ...
    if (existsSync(examplesPath)) {
      ...
      try {
        ...
        packageName = packageJson.name.replace('@godaddy/', '');
...
async function copyPackageDocs(sourceRoot, targetRoot) {
  const dirs = ['plugins', 'presets', 'modules', 'templates'];
  const excludedPath = '@godaddy';
...
async function copyRootDocs(projectRoot, targetRoot) {
  ...
  // Copy the README and CONTRIBUTING files
